...
@router.get("/")
async def get_holidays(current_user=Depends(get_current_tenant_user)):
    db = get_database()
    holidays = await db.holidays.find({"organization_id": current_user["organization_id"]}).sort("date", 1).to_list(500)
    return [serialize(h) for h in holidays]

@router.get("/upcoming")
async def get_upcoming_holidays(limit: int = 5, current_user=Depends(get_current_tenant_user)):
    db = get_database()
...
async def get_holidays_for_year(year: int, current_user=Depends(get_current_tenant_user)):
    db = get_database()
    holidays = await db.holidays.find({
...
async def create_holiday(data: HolidayCreate, current_user=Depends(get_current_admin)):
    db = get_database()
    existing = await db.holidays.find_one({"date": data.date, "organization_id": current_user["organization_id"]})
    if existing:
...
async def update_holiday(holiday_id: str, data: HolidayUpdate, current_user=Depends(get_current_admin)):
    ...
    result = await db.holidays.update_one(
        {"_id": ObjectId(holiday_id), "organization_id": current_user["organization_id"]},
        {"$set": update_dict}
    )
