...
async def get_all_leaves(current_user=Depends(get_current_admin)):
    db = get_database()
    leaves = await db.leaves.find().sort("created_at", -1).to_list(1000)
    return [serialize(l) for l in leaves]

@router.get("/my")
async def get_my_leaves(current_user=Depends(get_current_user)):
    db = get_database()
    leaves = await db.leaves.find({"employee_email": current_user["email"]}).sort("created_at", -1).to_list(100)
    return [serialize(l) for l in leaves]

@router.get("/pending")
async def get_pending_leaves(current_user=Depends(get_current_admin)):
    db = get_database()
    leaves = await db.leaves.find({"status": "pending"}).sort("created_at", -1).to_list(100)
    return [serialize(l) for l in leaves]
...
@router.post("/")
async def apply_leave(leave_data: LeaveCreate, current_user=Depends(get_current_user)):
    db = get_database()
...
async def update_leave_status(leave_id: str, update_data: LeaveUpdate, current_user=Depends(get_current_admin)):
    ...
    result = await db.leaves.update_one({"_id": ObjectId(leave_id)}, {"$set": update_dict})
...
async def cancel_leave(leave_id: str, current_user=Depends(get_current_user)):
    db = get_database()
    leave = await db.leaves.find_one({"_id": ObjectId(leave_id)})
    if not leave:
        raise HTTPException(status_code=404, detail="Leave not found")
